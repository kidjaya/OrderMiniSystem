...
//Route::get('/users','UserController@index'); //获取所有用户信息->判断管理员权限 <= 1  {{useless}}
...
Route::group(['middleware'=>'VerifyIdentity'],function(){
    Route::post('/goods/order/putOrder','OrderController@store');//插入订单 商家是否在线，库存是否足够
    Route::post('/goods/order/payOrder','OrderController@update');//支付订单 ->是否是当前用户 钱是否足够
    Route::post('/goods/myOrderList','OrderController@show');//获取当前用户订单每次加载10个
