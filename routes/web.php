...
//Route::group(['middleware'=>'haveUserId'],function(){
    ...
    Route::post('/users/addresses/add','UserAddressController@store');//增加用户地址->判断用户是否有user_id
    Route::post('/users/addresses/edit','UserAddressController@update');//修改用户地址->判断用户是否有user_id
    Route::post('/users/addresses/destroy','UserAddressController@destroy');//删除用户地址->判断用户是否有user_id
//});
...
/*********************************商品排序&商品详情&商品信息*******************************************/
Route::get('/sellers/cuisines/{seller_id}','SellerController@show');//获取商家所有商品
    Route::get('/sellers/sales/{seller_id}');//获取商家所有商品（按销量）
    Route::get('/sellers/price/{seller_id}');//获取商家所有商品（按价格）
...
//Route::group(['middleware'=>'haveUserId'],function(){
    Route::post('/goods/order/putOrder','OrderController@store');//插入订单 （提交订单->判断是否登录（有user_id），商家是否在线，库存是否足够
    Route::post('/goods/order/payOrder','OrderController@update');//支付订单 ->判断是否登录（有user_id）钱是否足够
